...
The approved script is aligned to the recorded transcript before treatment. A changed caption treatment invalidates render, QA, and package; it does not invalidate transcription or candidate selection. Short-native re-ingest does not invalidate the approved script.
...
The system stores facts, inferences, and judgments separately. Numbers and consequential factual claims enter `needs_review` and block treatment until the candidate ledger is edited with evidence and re-approved.
...
- Shortlisted windows are transcribed locally with faster-whisper INT8 and word timings.
- Final audio is normalized toward -14 LUFS and -1 dBTP.
- QA checks dimensions, frame rate, sample rate, loudness, true peak, duration, caption limits, canonical copy, frame perceptual hashes, and audio characteristics.
...
Feedback is captured from approvals, explicit comments, exact structured diffs, and re-imported external finals. External-video comparison records transcript, scene cuts, crop-sensitive frame fingerprints, duration, and loudness; optional SRT, EDL, and FCPXML sidecars preserve exact editor decisions.
