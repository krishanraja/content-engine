...
Bearer rotation and signing-key handling are different operations. The bearer may be rotated by stopping the task, changing the matching server and local bearer values, then restarting and reading back status. Protocol v1 does not support an in-place receipt-signing-key rotation: retained claims, receipts, project journals, conflicts, acknowledged cursors, and the external runner-authority marker have no key identifier and remain authenticated by the original key. A missing or changed signing key must fail closed without recreating the marker, identity, or authority roots. Restore the exact prior value. A deliberate change requires the schema-major migration or audited runner retirement described in [deployment](DEPLOYMENT.md). Never delete, edit, or re-sign local authority records by hand.
