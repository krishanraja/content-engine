...
Apply the publication format before choosing the proof rhythm. Money Trace, The Artifact, Verdict and Cold-open cutdown all require visible receipts and explicit claim boundaries. The Build Itself and First Version need an actual build or artifact early. Builder conversation and The Third Why may open on a revealing person or exchange when that human moment is the evidence. Do not force screenshots into those openings merely for consistency. The Artifact is the only public name; Teardown is a retired input alias.
...
Use only the official, content-addressed wordmarks declared in the pinned brand theme: the Mindmake wordmark and the matching Built With AI or The Money of AI composite wordmark from `krishanraja/mindmake/src/assets`. Never type, trace, approximate, recolour or regenerate a wordmark. Use the official ink-background variant for variable video footage, preserve its pixels, crop transparent padding only, and stack Mindmake above the matching series mark inside the single approved compact square in the top-left. Keep both marks at or above the configured legibility floor. A missing download, hash mismatch, absent series mapping, missing approved lockup or undersized mark is a hard render failure.
