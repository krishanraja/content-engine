...
Use only the official, content-addressed wordmarks declared in the pinned brand theme: the Mindmake wordmark and the matching Built With AI or The Money of AI composite wordmark from `krishanraja/mindmake/src/assets`. Never type, trace, approximate, recolour or regenerate a wordmark. Use the official ink-background variant for variable video footage, preserve its pixels, crop transparent padding only, and keep both marks on the verified contrast plate at or above the configured legibility floor. A missing download, hash mismatch, absent series mapping or undersized mark is a hard render failure.
