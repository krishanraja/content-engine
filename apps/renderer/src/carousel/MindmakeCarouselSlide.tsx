...
const BRAND_PLATE_PADDING_X = 20
const BRAND_PLATE_LEFT = CONTENT_LEFT - BRAND_PLATE_PADDING_X
const FOOTER_CLEARANCE = 128
...
function OfficialWordmark({ asset, width, lettersOnly = false }: { asset: Wordmark; width: number; lettersOnly?: boolean }) {
  ...
  const source = lettersOnly ? asset.letterCropDataUrl : asset.alphaCropDataUrl
  return <Img data-crop={lettersOnly ? 'letter-region' : 'alpha-crop'} src={source || asset.assetDataUrl || staticFile(asset.assetFile)} style={{ display: 'block', width, height }} />
}
...
function SignalRoom({ colors }: { colors: Colors }) {
  return <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
    ...
    <div style={{ position: 'absolute', left: 78, right: 78, bottom: FOOTER_CLEARANCE, height: 520, background: colors.raised, border: `2px solid ${colors.line}`, boxShadow: `18px 18px 0 ${MATERIAL.walnut}` }}>
...
function InspectionTable({ colors, items }: { colors: Colors; items: string[] }) {
  return <div style={{ position: 'absolute', left: 78, right: 78, bottom: FOOTER_CLEARANCE, height: 600, background: colors.ink, border: `2px solid ${colors.ink}`, boxShadow: `16px 16px 0 ${MATERIAL.walnut}` }}>
...
function EngravingBench({ colors, items }: { colors: Colors; items: string[] }) {
  return <div style={{ position: 'absolute', left: 0, right: 0, bottom: FOOTER_CLEARANCE, height: 660, background: colors.paper, borderTop: `2px solid ${colors.line}` }}>
...
function LeverCutaway({ colors, items }: { colors: Colors; items: string[] }) {
  return <div style={{ position: 'absolute', left: 56, right: 56, bottom: FOOTER_CLEARANCE, height: 650, border: `2px solid ${colors.ink}`, background: colors.paper, overflow: 'hidden' }}>
...
function XrayMismatch({ colors, items }: { colors: Colors; items: string[] }) {
  return <div style={{ position: 'absolute', left: 76, right: 76, bottom: FOOTER_CLEARANCE, height: 600 }}>
...
function ShutterCabinet({ colors, items }: { colors: Colors; items: string[] }) {
  return <div style={{ position: 'absolute', left: 56, right: 56, bottom: FOOTER_CLEARANCE, height: 650, background: MATERIAL.walnut, border: `2px solid ${colors.line}`, padding: 26, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 18 }}>
...
function OutputTray({ colors, items }: { colors: Colors; items: string[] }) {
  return <div style={{ position: 'absolute', left: 70, right: 70, bottom: FOOTER_CLEARANCE, height: 650 }}>
