 ...
 * Prompt A/B harness.
 *
 *   npx tsx scripts/eval/run.ts <suite> [options]
 *
 * Options
 ...
 *
 * Examples
 *   npx tsx scripts/eval/run.ts revise --dry --fixture scripts/eval/cases/revise.sample.json
 *   npx tsx scripts/eval/run.ts revise --cases 12 --repeats 2 --null
 */
import { loadEnv, requireEnv } from './_env.ts'
import { run, dry, type RunOpts } from './_runner.ts'
...
const SUITES: Record<string, () => Promise<{ suite: any }>> = {
  revise: () => import('./suites/revise.ts'),
}

if (!name || !SUITES[name]) {
  console.error(`usage: npx tsx scripts/eval/run.ts <${Object.keys(SUITES).join('|')}> [--dry] [--cases n] [--repeats n] [--null]`)
  process.exit(1)
}
