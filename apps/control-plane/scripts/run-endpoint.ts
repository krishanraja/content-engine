 ...
 * actually did.
 *
 *   npx tsx scripts/run-endpoint.ts <path-to-handler.ts> [query] [jsonBody]
 *
 * Example:
 *   npx tsx scripts/run-endpoint.ts api/discover-guest-scout.ts 'dry=1&format=paid'
 */
...
if (!handlerPath) {
  console.error('usage: run-endpoint.ts <handler.ts> [query] [jsonBody]')
  process.exit(1)
}
