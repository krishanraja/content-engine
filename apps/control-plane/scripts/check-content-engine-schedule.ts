...
//
// lib/contentEngineSchedule.ts is what the Content tab reads to say a job
...
//
//   npx tsx scripts/check-content-engine-schedule.ts
import { readFileSync } from 'node:fs'
import { CONTENT_ENGINE_JOBS } from '../lib/contentEngineSchedule.ts'
...
for (const c of vercel.crons || []) {
  ...
  if (m && !seen.has(m[1])) bad(`${file} records runs as '${m[1]}' but that job is not in lib/contentEngineSchedule.ts`)
}
