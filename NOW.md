...
- **The Studio knows the three subchannels.** A routed piece can get a production brief in its own name (2026-09-26). Krish approved the branding on 2026-09-26 (the makeyourmindup mark plus the channel name in mono); until it is built, a branded render for a live subchannel still refuses. The Windows runner is installed and reporting, but on `5f2db20` with Google Drive unavailable, so it needs updating to take live-name briefs (`docs/STUDIO.md`).
